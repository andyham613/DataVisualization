...
var interval = 0;
var curr_state;
var scale; // to update axis
/* information relevant to each interval */
var int_data = {
    0: { years: [0, 1967], domain: [0, 20000] },
    1: { years: [1968, 1980], domain: [0, 6000] },
    2: { years: [1981, 1997], domain: [0, 500] },
    3: { years: [1998, 2016], domain: [0, 200] }
}
...
function sortBySet(wat) {
    ...
    switch (sortBy) {
      case "mex":
         p3.stRate.sort(function(a, b){
           let arates = a.rates.filter(x => x.year <= int_data[interval].years[1] && x.year >= int_data[interval].years[0])
                              .map(x => x.rate);
           let brates = b.rates.filter(x => x.year >= int_data[interval].years[0] && x.year <= int_data[interval].years[1])
                              .map(x => x.rate);
    ...
    rows.selectAll("rect")
          ...
          .attr("id", function(d,i) {
            return d3.select(this.parentNode).attr("id") + "-" + d.year;
          })
          .transition(p3.transDur)
          .style("fill", function(d) {
            let val = d.rate;
            if (colorBy == "diff") {
              val = d.rate - p3.usRate[d.year - 1930].rate;
              if (isNaN(val)) {
                val = 0;
              }
            }
            return cmap(val).toString()});
...
function colorBySet(wat) {
  ...
  var colormap = d3.scaleLinear().domain(controls.map(intMap)).range(controls.map(base_colormap))
    ...
    var state_rate = p3.stRate.map( function(d){
      return {"state": d.state, "rate" : d.rates[selected_year - 1930].rate};
    });
    d3.selectAll(".stateHex")
      .data(p3.mapData)
      .transition(p3.transDur)
      .style("stroke-opacity", function(d) {
            if(d.StateAbbr == state){
                return 1;
            } else{return 0;}
      })
      ...
      .style("fill", function(d){
        ...
        // }
        // if (colorBy == "diff") {
        //     return diff.c(diff.scale(state_rate.filter(function(x){
        //       return x[0] === d.StateAbbr
        //     })[0].rate - find(selected_year)))
        // }
        if (colorBy == "diff") {
            return diff.c(diff.scale(state_rate.filter(s => (s.state == d.StateAbbr))[0].rate - find(selected_year)))
        }
      });
}
...
function finalFinish() {
  ...
  scale = d3.scaleLinear().domain(int_data[interval].domain).range([p3.plotHght, 0]);
  d3.select("#plot-us")
...
function yearSelect(year) {
    ...
    d3.selectAll(".stateHex")
      ...
      .transition(p3.transDur)
      .style("fill", function(d){
        return cmap(state_rate.filter(function(x){
          return x[0] === d.StateAbbr
        })[0][1])
      })
}
...
function scaleInt(interval) {
   return d3.scaleLinear()
            .domain(int_data[interval].domain)
            .range([p3.plotHght, 0]);
}

/* cmap(): creates a divergent colormap,
   based on values or value-mean rate data */
function cmap(rate) {
  if (colorBy == "value") {
    return d3.hcl(d3.scaleLinear().range([330, 0])
                                  .domain([0, 30000])(rate),
                  d3.scaleLog().range([0, 30]).domain([1, 2])(d3.scaleLinear().range([0, 1]).domain([0, 30000])(rate) + 1),
                  d3.scaleLog().range([0, 100]).domain([1, 30001])(rate + 1));
  } else {
    let logit = function(x) {
      return 1 / (1 + Math.exp(-0.05 * x));
    }
    let s = function(x) {
      return 0.4 * Math.pow(Math.cos(Math.PI * x), 2);
    }
    let l = function(x) {
      return 0.3 * Math.pow(Math.cos(Math.PI * x), 2) + 0.3;
    }
    return d3.hsl(d3.scaleQuantize().domain([0, 1]).range([0, 150, 100])(logit(rate)), s(logit(rate)), l(logit(rate)));
  }
}
...
function onMouse(ID, xy) {
    ...
    if (IDspl[0] === "plot") {
      ...
      console.log("searching for p3 search link here")
    }
    interval = getInterval(year);
   ...
   d3.select("#plotsG")
       .selectAll("path")
         .attr("stroke", d3.rgb(0,0,0,p3.plotOpac));
   /* updates plot and grid on state selection/deselection */
   if (state != 'bkgd') {
      if (state != curr_state || curr_state == 'none') {
   ...
   d3.select("#plotUSG")
       .call(d3.axisLeft(scale.domain(int_data[interval].domain)));
   // updating plots based on scale for current interval
   for (let i = 0; i < p3.numStates; i++) {
      let stateData = p3.stRate[i];
      d3.select('#plot-' + stateData.state)
          .datum(d => stateData.rates).transition(p3.transDur)
    ...
    d3.select("#plot-us")
        ...
        .attr("stroke", "orange");
   sortBySet(sortBy);
   console.log(p3.searchLink(IDspl[2],IDspl[1]));
   d3.select("#stateMapLink").html(p3.searchLink(IDspl[2],IDspl[1]));
